...
alz <- alz |>
  filter(games != 8)
chess_graph =  ggplot(data = alz, aes(x = as.factor(games), fill = as.factor(diagnosis))) + labs(title = "Amount of Difficulty Subject Faced Performing a Game or Hobby based on Alzheimer Diagnosis", x = "Difficulty Level", y = "Number of Patients")+geom_bar()+ theme(plot.title = element_text(size = 10))
chess_graph
